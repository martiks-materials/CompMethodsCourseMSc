...
void Rungekutta4::step(std::valarray<double> &ys , double xs, double d, bool reuse, bool single, bool endhalf){
...
void Rungekutta4::iterate(){
	...
	Rungekutta4::step(y_c, x_now, h, true, true, false);
        ...
        double h_new = h*safety*std::pow(std::abs(eps*y_c.max()/delta1.max()), 0.2);
